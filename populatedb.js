...
function createCategories(cb) {
  async.series(
    [
      function (callback) {
        categoryCreate('dairy', 'made from other mammals mammary glands?', callback);
      },
      function (callback) {
    ...
    itemsToAdd.map(
      (item) =>
        function (callback) {
          ...
          var categoryIndex = categories.indexOf(categories.find((cat) => cat.name == category));
          itemCreate(name, description, price, stock, categories[categoryIndex], callback);
        }
    ),
    // optional callback
    cb
